...
class Eh_Bacs extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = sprintf( __( 'Pay with Bacs Direct Debit', 'payment-gateway-stripe-and-woocommerce-integration' ) );
		$this->supports           = array(
			'products',
			'refunds',
		);
	...
	public function get_icon() {
        ...
        $icon_show = apply_filters('eh_show_stripe_checkout_icon', false);
        ...
        if($icon_show){
            ...
            $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/google-pay.png') . '" alt="Apple Pay" width="32" title="Apple Pay" ' . $style . ' />';
        }
    ...
    public function payment_scripts(){
        ...
        if(is_checkout()){ 
            ...
            wp_localize_script( 'eh_checkout_script', 'eh_bacs_params', $eh_bacs_params);
        }
    }
    ...
    public function eh_bacs_stripe_checkout_order_callback() { 
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'cancel_order') { 
            ...
            if (isset($payment_intent->status) && $payment_intent->status == 'processing') {
               ...
               $order->add_order_note(__('Wait for the payment to succeed or fail.', 'payment-gateway-stripe-and-woocommerce-integration'));
                ...
                $data = $obj->make_charge_params($charge_response, $order_id);print '<pre>';print_r($payment_intent);
