...
class Eh_Stripe_Checkout extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
            ...
            'eh_send_line_items'      => array(
                ...
                'type'        => 'checkbox',
                'default'     => 'no',
                //'desc_tip'    => __('Enable to send line items to Stripe Checkout page.','payment-gateway-stripe-and-woocommerce-integration'),
            ),              
		);   
    }
	...
	public function process_payment( $order_id ) {
        ...
        //$display_line_items = apply_filters('wt_show_line_items_on_ceckout', false);
        if($display_line_items === 'yes'){ 
               ...
               if (WC()->cart->display_cart_ex_tax) {
                ...
                if(isset($featured_image[0])){
                    $session_data['line_items'][] = array(
                        ...
                        'amount' => EH_Stripe_Payment::get_stripe_amount( round( $item_amount / $item->get_quantity(), 2), $currency ),
                        'images'   => array($featured_image[0]),
                    );            
                }
                else{
                    $session_data['line_items'][] = array(
            ...
            if ( 0 < $order->get_shipping_total() ) {
                 $session_data['shipping_options'][] = array(
                    'shipping_rate_data' => array(
                        'display_name'        => __('Shipping', 'payment-gateway-stripe-and-woocommerce-integration'),
                        'fixed_amount'    => array(
                            'amount'      => EH_Stripe_Payment::get_stripe_amount( $order->get_shipping_total(), $currency ),
                            'currency'    => $currency,
                        ),
                        'type' => __( 'fixed_amount', 'payment-gateway-stripe-and-woocommerce-integration' ),
                    )
                );
            } 
            ...
            if ( 0 < $order->get_discount_total() ) { 
               ...
               $coupons = \Stripe\Coupon::create([
                    ...
                    'currency'    => $currency,
                    'name'        => __( 'Discount', 'payment-gateway-stripe-and-woocommerce-integration' ),
                ]); 
                ...
                if(defined("WAD_DIR")){
                    if (file_exists(WAD_DIR . '/wad.php')) {
                       ...
                       if(!empty($discounts) && isset($discounts->id)){
                        ...
                        $session_data['line_items'][] = array(
                            'name'        => __( 'Fee', 'payment-gateway-stripe-and-woocommerce-integration' ),
                            'amount'      => EH_Stripe_Payment::get_stripe_amount($fee_total, $currency ),
                            'currency'    => $currency,
                            'quantity'    => 1,
                        );                    
                    }
            ...
            if(WC()->cart->display_cart_ex_tax){
                ...
                if ( 0 < $order->get_total_tax() ) {
                     $session_data['line_items'][] = array(
                        'name'        => __( 'Tax', 'payment-gateway-stripe-and-woocommerce-integration' ),
